...
if __name__ == "__main__":
    ...
    # IMPORT MG OUTPUT
    cols = "t sec, xC m, yC m, zC m, xD m, yD m, zD m, xF m, yF m, zF m, qA deg, qB deg, qC deg, qD deg, qF deg, ME J"

    df = pd.read_csv('MGLeftRigidTwoWheels.1',
    ...
    # Front wheel
    vis["F"].set_object(g.Cylinder(WHEEL_THICKNESS, R))
    vis["F_tick"].set_object(g.Box([R*1.7, R/2, R/4]),
                             g.MeshBasicMaterial(color=0xff0000))
    ...
    for i in tqdm(range(0, df.shape[0])):
        ...
        with anim.at_frame(vis, i) as frame:
            ...
            B_R_F = np.array([[np.cos(qF), 0, np.sin(qF)],
                              [0, 1.0, 0],
                              [-np.sin(qF), 0, np.cos(qF)]])

            N_R_C = N_R_A @ A_R_B @ B_R_C
            N_R_D = N_R_A @ A_R_B @ B_R_D
            N_R_F = N_R_A @ A_R_B @ B_R_F
            ...
            F_transform = np.eye(4)
            F_transform[0:3, 0:3] = N_R_F
            F_transform[0:3, 3] = np.array([xF, yF, zF])
            frame["F"].set_transform(F_transform)
            frame["F_tick"].set_transform(F_transform)
