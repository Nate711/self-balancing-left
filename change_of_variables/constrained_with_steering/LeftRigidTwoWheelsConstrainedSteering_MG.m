...
Variable  qF''
Variable  wG'                     % Angular speed of front wheel G in steering column F
Constant  r = 0.1 m               % Radius of disk 
Constant  TRAIL = 0 m
...
F.RotatePositiveZ( E, qF )
G.SetAngularVelocity( F, wG * Fy>)
...
% Steering column
Fo.Translate( Eo, LG * Ex> )

% Front wheel
Gcm.Translate( Fo, -TRAIL * Fx> )

% WITH TRAIL
lambda = sqrt(Dot(Az>, Fx>)^2 + Dot(Az>, Fz>)^2) / (2 * r)
GAx = Dot(Az>, Fx>) / (2 * lambda)
GAz = - Dot(Az>, Fz>) / (2 * lambda)
r_Gcm_GA> = GAx * Fx> + GAz * Fz> 
GA.SetPositionVelocity( Gcm, r_Gcm_GA> ) % WHAT VECTOR IS DOWN

% WITH ZERO TRAIL
% GA.SetPositionVelocity( Gcm, -r * Fz> ) % WHAT VECTOR IS DOWN. Fz is incorrect even with no trail b/c of steering angle
