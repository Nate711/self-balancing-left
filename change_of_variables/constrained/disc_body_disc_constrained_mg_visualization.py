...
if __name__ == "__main__":
    ...
    # IMPORT MG OUTPUT
    cols = "t sec, xD m, yD m, zD m, xE m, yE m, zE m, xG m, yG m, zG m, qH deg, qL deg, qS deg, qE deg, qG deg, ME J"

    df = pd.read_csv('MGLeftRigidTwoWheelsConstrained_MG.1',
    ...
    # Back wheel
    vis["G"].set_object(g.Cylinder(WHEEL_THICKNESS, R))
    vis["G_tick"].set_object(g.Box([R*1.7, R/2, R/4]),
                             g.MeshBasicMaterial(color=0xff0000))
    ...
    for i in tqdm(range(0, df.shape[0])):
        ...
        with anim.at_frame(vis, i) as frame:
            ...
            C_R_G = np.array([[np.cos(qG), 0, np.sin(qG)],
                              [0, 1.0, 0],
                              [-np.sin(qG), 0, np.cos(qG)]])

            A_R_D = A_R_B @ B_R_C @ C_R_D
            A_R_E = A_R_B @ B_R_C @ C_R_E
            A_R_G = A_R_B @ B_R_C @ C_R_G
            ...
            G_transform = np.eye(4)
            G_transform[0:3, 0:3] = A_R_G
            G_transform[0:3, 3] = np.array([xG, yG, zG])
            frame["G"].set_transform(G_transform)
            frame["G_tick"].set_transform(G_transform)
